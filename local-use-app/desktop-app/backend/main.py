...
def start_app():
    ...
    def on_window_ready():
        logger.info("ShadowLith Window Ready.")
        
        def stealth_init():
            try:
                time.sleep(1.0)
                api.window.apply_stealth()
                api.window.set_ghost_style(interactive=True)
            except Exception as e:
                logger.error(f"Stealth Init Error: {e}")
            
        threading.Thread(target=stealth_init, daemon=True).start()
