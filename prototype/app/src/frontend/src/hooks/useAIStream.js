    ...
    useEffect(() => {
        ...
        window.__onStreamEnd = (streamId) => {
            ...
            try {
                ...
                // Fallback: If streaming text is just plain text, push it as a chat block
                if (!finalResult.includes("{") && finalResult.trim().length > 0) {
                     onChatResult([{ type: 'text', content: finalResult.trim() }], false);
                     return;
                }
