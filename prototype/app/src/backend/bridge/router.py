...
class ShadowLithAPI:
    ...
    def toggle_ui(self):
        if self._window_ref:
            ...
            if not is_visible:
                # CRITICAL: Apply stealth BEFORE showing to prevent even a single 
                # visible frame from being captured by screen sharing software.
                _hwnd = self.window.get_hwnd()
                self.window.apply_stealth(hwnd=_hwnd)
                self.window.set_ghost_style(interactive=True, hwnd=_hwnd)
                # Now safe to show — window surfaces already cloaked
                self._window_ref.show()
                self._window_ref.restore()
                self.window.show()
                # Re-apply once more after show in case Windows reset during show()
                self.window.apply_stealth(hwnd=_hwnd)
                logger.info("UI Visibility toggled: Shown (Stealth pre-applied)")
            else:
                # Need to hide
                self._window_ref.hide()
                logger.info("UI Visibility toggled: Hidden")
