...
class VisionService:
    ...
    def capture_with_snip_ocr(self):
        ...
        try:
            ...
            img_path = str(Config.SCREENSHOTS_DIR / "last_snip.png")
            
            # Use --snapper flag so the bundled EXE knows to run the snip tool logic
            if getattr(sys, 'frozen', False):
                result = subprocess.run([sys.executable, "--snapper", img_path], capture_output=True, text=True)
            else:
                result = subprocess.run([sys.executable, str(Config.SNAPPER_PATH), img_path], capture_output=True, text=True)
