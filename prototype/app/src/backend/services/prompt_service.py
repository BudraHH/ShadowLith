...
class PromptService:
    ...
    def _update_system_instruction(self):
        self._system_instruction = (
            ...
            "   }\n\n"
            "JSON SAFETY RULES:\n"
            "1. NO DOUBLE QUOTES: Do NOT use double quotes (\") inside any 'content' string. Use SINGLE QUOTES (') for ALL literals, code characters, and strings. \n"
            "2. EXAMPLE: 'content': 'print(\\'Hello World\\')' is CORRECT. 'content': 'print(\"Hello World\")' is FORBIDDEN.\n"
            "3. ESCAPING: If you absolutely cannot avoid a double quote, you MUST escape it with a double backslash: \\\\\".\n\n"
    ...
    def build_analysis_prompt(self, mode, language, scenario, transcript=None, is_audit=False):
        ...
        base_instruction = ""
        context_suffix = f"\n\nADDITIONAL TEXT CONTEXT:\n{transcript}" if (transcript and transcript.strip()) else ""
        ...
        if is_audit:
            return (
                ...
                f"Keep it concise. Point out specific errors or give a hint.{context_suffix}"
            )
        
        if mode == "Assessment":
            if scenario == "Coding":
                base_instruction = f"MODE: Assessment | SCENARIO: Coding | LANGUAGE: {language}\nOutput problem, strategy, and code blocks."
            elif scenario == "MCQ":
                base_instruction = "MODE: Assessment | SCENARIO: MCQ\nSOLVE the question. Identify the SINGLE correct answer. Output ONE AND ONLY ONE 'option' block containing the correct answer, then output the problem block. DO NOT transcribe or list the incorrect options."
            else:
                base_instruction = "MODE: Assessment | SCENARIO: Video\nOutput problem and interview (professional script) blocks."
            return base_instruction + context_suffix
        
        # Interview Mode
        base_instruction = f"MODE: {mode} | INTERVIEW TYPE: SMART (Technical/Behavioral)\nPriority #1 is the 'interview' block (the script)."
        return base_instruction + context_suffix

    def build_chat_prompt(self, message, is_interview_mode=False):
        safety_append = "\n\nCRITICAL: Respond ONLY in the requested JSON string format. Never use markdown formatting. ALL normal conversational text MUST go inside a {'type': 'text', 'content': 'YOUR TEXT HERE'} block."
        if is_interview_mode:
            return f"INTERVIEWER_QUESTION: {message}\nTASK: Answer the interviewer directly.{safety_append}"
        return f"USER_CHAT: {message}\nTASK: Process the user's request.{safety_append}"
