...
class SnapperWin(QtWidgets.QWidget):
    def __init__(self, save_path=None):
        ...
        print(f"DEBUG: SnapperWin Initializing... Output Path: {self.save_path}")
    ...
    def mouseReleaseEvent(self, event):
        if event.button() == QtCore.Qt.MouseButton.LeftButton and self.is_selecting:
            ...
            if rect.width() > 10 and rect.height() > 10:
                ...
                # Save to specific path passed via args or fallback to default
                if self.save_path:
                    dest = self.save_path
                else:
...
if __name__ == "__main__":
    ...
    window = SnapperWin(save_path=save_path)
