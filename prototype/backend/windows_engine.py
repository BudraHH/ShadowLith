...
class WindowsEngine:
    ...
    def set_ghost_style(self, hwnd: int, interactive: bool = False) -> bool:
        """ 
        Applies ToolWindow style to hide from Taskbar and Alt+Tab.
        WS_EX_NOACTIVATE prevents the window from stealing focus when False.
        """
        if not self.is_windows: return False
        try:
            ...
            # 1. Get current ex-style
            current_ex_style = win32gui.GetWindowLong(hwnd, win32con.GWL_EXSTYLE)
            
            # 2. Modify: Add TOOLWINDOW, remove APPWINDOW
            new_ex_style = current_ex_style | win32con.WS_EX_TOOLWINDOW
            new_ex_style &= ~win32con.WS_EX_APPWINDOW
            
            # 3. Handle NOACTIVATE based on interactivity
            if interactive:
                new_ex_style &= ~0x08000000 # Remove WS_EX_NOACTIVATE
            else:
                new_ex_style |= 0x08000000  # Add WS_EX_NOACTIVATE
            
            win32gui.SetWindowLong(hwnd, win32con.GWL_EXSTYLE, new_ex_style)
            
            # 4. Force window update
            win32gui.SetWindowPos(hwnd, 0, 0, 0, 0, 0, 
                win32con.SWP_NOMOVE | win32con.SWP_NOSIZE | win32con.SWP_NOZORDER | win32con.SWP_FRAMECHANGED)
            
            print(f"Ghost Style Updated: Hidden from Taskbar. Interactive: {interactive}")
            return True
        except Exception as e:
            print(f"Failed to apply Ghost Style: {e}")
            return False
